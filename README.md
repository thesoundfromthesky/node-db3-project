...
- In [SQL Try Editor at W3Schools.com](https://www.w3schools.com/Sql/tryit.asp?filename=trysql_select_top):
  ...
  - Displays CategoryName and a new column called Count that shows how many products are in each category. Shows 8 records.
    SELECT categoryname, count(\*) as Count FROM [Products] as p join categories as c on p.categoryid = c.categoryid group by categoryname;

  - Display OrderID and a column called ItemCount that shows the total number of products placed on the order. Shows 196 records.
    select orderid, count(\*) as ItemCount from orderdetails group by orderid order by orderid asc;
